...
def generate_pdf(output_filepath, data):
    ...
    table1 = Table(table1_data, colWidths=[1.16*inch, 0.85*inch, 0.85*inch, 0.5*inch, 1.16*inch, 1.16*inch, 1.1*inch, 0.6*inch])
    ...
    table2 = Table(table2_data, colWidths=[col1_width_t2, col2_width_t2, col3_width_t2, col4_width_t2, col5_width_t2, col6_width_t2, 1.0*inch, col8_width_t2], hAlign='LEFT')
    ...
    ('BACKGROUND', (3, 5), (4, 5), colors.lightgrey), # Shade Total a Depositar and its value
    ...
    if data.get('imprimir_comision_afiliacion', False):
        ...
        comision_afiliacion_data = [
            ...
            [Paragraph("<b>COMISION</b>", styles['BoldSmallFont']), Paragraph(f"PEN {data.get('comision_afiliacion_comision', '')}", styles['BoldSmallFontRight'])],
            [Paragraph("<b>IGV</b>", styles['BoldSmallFont']), Paragraph(f"PEN {data.get('comision_afiliacion_igv', '')}", styles['BoldSmallFontRight'])],
            [Paragraph("<b>TOTAL</b>", styles['BoldSmallFont']), Paragraph(f"PEN {data.get('comision_afiliacion_total', '')}", styles['BoldSmallFontRight'])],
        ]
    ...
    if os.path.exists(foxit_reader_path):
        try:
            import subprocess
            subprocess.Popen([foxit_reader_path, output_filepath], shell=False) # Changed output_file to output_filepath
            print(f"Attempting to open PDF with Foxit Reader: {output_filepath}")
